...
It ships **two intentional changes that must survive every upstream sync**.
Both are small, localized edits in `index.ts`. Upstream does not (and should
...
2. **Conflict in any tool description / promptSnippet / field description** → keep the fork's short version (run `git show 5a574ed -- index.ts` for the exact strings).
3. **Upstream removed or restructured something we edited (e.g. moved the whole description block)** → re-apply the fork change on top of the new structure; the anchors below will tell you if it survived.
4. **Never** resolve by force-push, rebase, or `git reset` on `main` — merge history keeps future conflicts small.
...
All four must match. If any do not, re-apply the corresponding delta
