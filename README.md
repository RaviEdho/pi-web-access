...
| `provider` | Configured provider when omitted or set to `auto`; otherwise `openai`, `brave`, `parallel`, `tavily`, `searxng`, `exa`, `perplexity`, or `gemini` (auto-selects when no provider is configured) |
